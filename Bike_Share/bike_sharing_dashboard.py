...
def plot_monthly_trends():
    ...
    monthly_counts['mnth'] = pd.Categorical(monthly_counts['mnth'], categories=month_order, ordered=True)
    plt.figure(figsize=(10, 6))
...
def plot_user_type_comparison():
    ...
    plt.title('Perbandingan Penyewaan Sepeda antara Pengguna Terdaftar dan Tidak Terdaftar')
    plt.ylabel('Jumlah Penyewaan')
    plt.xlabel('Tipe Pengguna')
    plt.tight_layout()  # Untuk memastikan tata letak tidak tumpang tindih
    st.pyplot(plt)
...
st.header("2. Jumlah total sepeda yang disewakan berdasarkan Bulan dan tahun 2014 2015")
st.write(
